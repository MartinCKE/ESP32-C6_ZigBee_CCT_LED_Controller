...
{
    uint16_t sum = (uint16_t)s_out_amber + (uint16_t)s_out_white;
    if (sum > 255) sum = 255;
    return (uint8_t)sum;
}
...
void led_apply_brightness_and_ct(uint16_t brightness, uint16_t mired)
{
    ...
    // Convert ratio to fixed-point mix [0..255]
    int mix = (int)lroundf(white_ratio * 255.0f);
    if (mix < 0) mix = 0;
    if (mix > 255) mix = 255;

    // Compute white with rounding, amber as remainder (conserves brightness)
    uint16_t white16 = (uint16_t)brightness * (uint16_t)mix; // up to 255*255
    uint8_t white_pwm = (uint8_t)((white16 + 127) / 255);

    uint8_t amber_pwm = (brightness >= white_pwm)
                        ? (uint8_t)(brightness - white_pwm)
                        : 0;
